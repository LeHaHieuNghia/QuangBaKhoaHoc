import NguyenMinhTri from "../assets/Nguyen-Minh-Tri.jpg";
import TranMyLan from "../assets/Tran-My-Lan.jpg";
import PhamQuocHuy from "../assets/Pham-Quoc-Huy.jpg";
import LeThiMyDuyen from "../assets/Le-Thi-My-Duyen.jpg";

export const UserData = [
  {
    id: 1,
    img: NguyenMinhTri,
    name: "Nguyễn Minh Trí",
    position: "Chuyên viên Marketing",
    description:
      "Trước khoá học, mình hoàn toàn mù mờ về AI. Nhưng sau 3 ngày học tập cùng giảng viên của Suri Technologies, mình không chỉ hiểu được các khái niệm nền tảng mà còn biết cách ứng dụng AI vào công việc thực tế như tạo chatbot, phân tích dữ liệu và tối ưu quy trình marketing.",
  },
  {
    id: 2,
    img: TranMyLan,
    name: "Trần Thị Lan",
    position: "Chủ shop online tại Đà Lạt",
    description:
      "Ấn tượng nhất là phần thực hành xây dựng chatbot AI. Mình đã tạo được một AI Agent demo cho doanh nghiệp nhỏ của mình chỉ sau vài giờ. Khoá học thật sự đáng giá với những ai đang muốn bắt đầu hành trình chuyển đổi số.",
  },
  {
    id: 3,
    img: PhamQuocHuy,
    name: "Phạm Quốc Huy",
    position: "Nhân viên văn phòng",
    description:
      "Khoá học được tổ chức bài bản, tài liệu đầy đủ, giảng viên hỗ trợ tận tâm cả trong và sau buổi học. Mình đặc biệt thích phần ứng dụng ChatGPT vào công việc văn phòng, rất thiết thực!",
  },
  // {
  //   id: 4,
  //   img: LeThiMyDuyen,
  //   name: "Lê Thị Mỹ Duyên",
  //   position: "Nhân viên kinh doanh tự do",
  //   description:
  //     "Lúc đầu mình nghĩ AI là thứ gì đó rất xa vời với người không có nền tảng công nghệ như mình. Nhưng khoá học đã giúp mình thay đổi hoàn toàn suy nghĩ đó. Nhờ phần hướng dẫn chi tiết và bài tập thực tế, mình đã biết cách sử dụng AI để tạo nội dung, lên kế hoạch bài đăng mạng xã hội và hỗ trợ chăm sóc khách hàng tự động.",
  // },
];
